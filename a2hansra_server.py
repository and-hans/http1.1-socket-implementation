...
def main():
    ...
    """
    The way this is set up within the try/catch blocks allows the server to continualy process requests.
    This is the expected behaviour. You should write additional functions (like one function per verb), but
    you should keep this basic structure. If you don't, then a bad request may make it so that your server
    stops responding to requests.

    During testing, our client code will provide requests one by one, but your server file will be run only
    once. If it shuts down or otherwise gets stuck, no further requests can be graded.
    """
